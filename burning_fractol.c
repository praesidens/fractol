...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/10/14 20:44:03 by bsuprun           #+#    #+#             */
/*   Updated: 2018/10/14 20:44:04 by bsuprun          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		burning_threads(t_fractol *fract)
{
	...
	j = 0;
	while (++j <= THREADS)
		pthread_create(&threads[j - 1], NULL, burning_helper, &storage[j - 1]);
	while (j--)
...
void		burning(t_fractol *f)
{
	...
	while (i < f->max_iter)
	{
		f->o_r = f->n_r;
		f->o_i = f->n_i;
		f->n_r = fabs(f->o_r * f->o_r - f->o_i * f->o_i) + f->p_r;
