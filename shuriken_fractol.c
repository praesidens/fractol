...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/10/15 02:14:08 by bsuprun           #+#    #+#             */
/*   Updated: 2018/10/15 02:14:09 by bsuprun          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		shuriken_threads(t_fractol *fract)
{
	...
	j = 0;
	while (++j <= THREADS)
		pthread_create(&threads[j - 1], NULL, shuriken_helper, &storage[j - 1]);
	while (j--)
...
void		shuriken(t_fractol *f)
{
	...
	if (i < f->max_iter)
		put_pixel_to_image(f, f->x, f->y, find_color_4(f, i));
	else
