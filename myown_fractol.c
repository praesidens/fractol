...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/10/14 20:45:13 by bsuprun           #+#    #+#             */
/*   Updated: 2018/10/14 20:45:14 by bsuprun          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		myown_threads(t_fractol *fract)
{
	...
	j = 0;
	while (++j <= THREADS)
		pthread_create(&threads[j - 1], NULL, myown_helper, &storage[j - 1]);
	while (j--)
...
void		myown(t_fractol *f)
{
	...
	while (i < f->max_iter)
	{
		f->o_r = f->n_r;
		f->o_i = f->n_i;
		f->n_r = f->o_r * f->o_r * f->o_r - f->o_i * f->o_i + f->p_r;
		f->n_i = 0.5 * f->o_r * f->o_i * f->o_r * f->o_i + f->p_i;
		i++;
		if ((f->n_r * f->n_r + f->n_i * f->n_i) >= 16)
			break ;
	}
	if (i < f->max_iter)
		put_pixel_to_image(f, f->x, f->y, find_color_3(f, i));
	else
